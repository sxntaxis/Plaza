...
Plaza se alinea con los estándares internacionales establecidos para cada capa: ELI para identificadores, metadata legal, modelo FRBR y relaciones normativas; PROV-O para procedencia; SKOS para vocabularios controlados; schema.org/Legislation para interoperabilidad web; DCAT para catálogos de datos; SHACL para validación formal; W3C ORG para estructuras institucionales futuras; y Akoma Ntoso para serialización XML documental de textos legales. Estos no son objetivos futuros — son la columna sobre la cual se construye cada capa del modelo.

Estos estándares no fueron elegidos por preferencia estética. Son los que actualmente usa la mayor parte del mundo legal y de datos abiertos — la Unión Europea publica su derecho en ELI, varios parlamentos publican documentos en Akoma Ntoso, y los catálogos de datos abiertos de gobiernos en media docena de países siguen DCAT. Al adoptarlos por capa, Plaza se vuelve interoperable sin mezclar responsabilidades: los sistemas externos no necesitan aprender el dialecto de Costa Rica, y los desarrolladores costarricenses que trabajan con Plaza aprenden vocabularios que les sirven globalmente.
